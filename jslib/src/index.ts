...
import * as Realm from './types/Realm';

const globalRealm = (<any>this).Realm;

declare const jest: any;
declare const __DEV__: boolean;

/** Pull in typescript types */
import pathType from 'path';
import { setRealmConstructor, getRealmConstructor } from './resolveRealmConstructor';
...
function getContext() {
    ...
    if (typeof process === 'object' && process + '' === '[object process]') {
        ...
        if (typeof global !== 'undefined' && (<any>global).__debug__) {
            return 'vscodedebugger';
        }

        return (<any>process).type === 'renderer' ? 'electron' : 'node.js';
    }
    ...
    if (typeof navigator !== 'undefined' && navigator.product === 'ReactNative') { // eslint-disable-line no-undef
        ...
        if (typeof globalRealm === 'undefined') {
...
switch(context) {
    case 'node.js':
    case 'electron':
        nodeRequire<any>('./submit-analytics')('Run', context);

        var binary = nodeRequire<any>('node-pre-gyp');
        var path = nodeRequire<typeof pathType>('path');
        var pkg = path.resolve(path.join(__dirname,'../package.json'));
        var binding_path = binary.find(pkg);

        setRealmConstructor(nodeRequire<any>(binding_path).Realm);
        break;

    case 'reactnative':
    case 'jscore':
        setRealmConstructor(globalRealm);  // eslint-disable-line no-undef
        break;
    ...
    case 'vscodedebugger':
        ...
        if (__DEV__) {
            setRealmConstructor(require('./browser').default); // (exported as ES6 module)
        } else {
...
if (realmConstructor.Sync) {
    if (context === 'node.js') {
      nodeRequire<any>('./notifier')(realmConstructor);
      if (!(<any>realmConstructor).Worker) {
          Object.defineProperty(realmConstructor, 'Worker', {value: nodeRequire<any>('./worker')});
      }
    }
}
