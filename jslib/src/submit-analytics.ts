...
function sha256(data: string | NodeJS.ArrayBufferView) {
    let hash = (<typeof cryptoType>require('crypto')).createHash('sha256');
    hash.update(data);
    return hash.digest('hex');
}

let mainOutput: (eventName: string, context: string) => void;

if (isAnalyticsDisabled()) {
    mainOutput = function(){};
} else {
    const os: typeof osType = require('os');
    const request: typeof requestType = require('request');
    const { machineId } = require("node-machine-id") as { machineId(original?: boolean) : Promise<string> };

    mainOutput = function(eventName, context) {
